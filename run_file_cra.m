...
for id = 1
    ...
    for i=1:length(lambda_set)
        parfor iter=1:n_iters
            % main algorithm
            tic;
            [H, obj] = mc_cra(X, k, lambda_set(i), maxiter);
            % final H 
            H = reshape(H, size(H,1), numel(H)/size(H,1));
            [y] = my_kmeans(H, k);
            ts = toc;
            [eval] = my_eval_y(y, Y);
            fprintf('\nlambda: %d, iter: %d, time: %f, acc: %f, nmi: %f, pur: %f', i, length(obj{1}), ts, eval(1), eval(2), eval(3)); 
            res(i,iter,:) = eval;
            runtime(i, iter) = ts;
            objs{i, iter} = obj;
        end
    ...
    save([proj_path, 'res_cra/', data_name, '_res.mat'], 'data_name', 'lambda_set', 'res', 'runtime', 'objs');
